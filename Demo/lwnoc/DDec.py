...
class DDecReq(Component):
    ...
    def __init__(self, node):
        ...
        self.mst_id_list = [self.create('out%s_id' % i, Output(UInt(master_id_width)))  for i in range(num)]
        self.slv_id_list = [self.create('out%s_id' % i, Output(UInt(slave_id_width)))   for i in range(num)]
        ...
        for i, dst in enumerate(self.node.dst_list):
            ...
            for gid in dst.global_master_id_list:
                ...
                id_hit += Equal(self.in0_mst_id, UInt(master_id_width, gid))
