...
class TemplateComponent(VComponent):
    ...
    # Class-level set to track TemplateIPs that have been included in filelist
    # This is reset at the start of each top-level filelist generation
    _filelist_template_ips = set()
    ...
    def _generate_filelist_core(self, prefix=''):
        ...
        we reference the TemplateIP's own filelist using the -f directive.
        
        This method ensures:
        1. Each TemplateIP's filelist is included only once (singleton deduplication)
        2. The filelist path is relative to the wrapper's output directory
        3. TemplateComponent itself doesn't generate a .v file entry
        
        Args:
            prefix: Path prefix for generated files
        
        Returns:
            List containing the -f directive for this TemplateIP's filelist
        """
        name_list = []
        template_ip = self.parent_template
        
        # Only add filelist if we haven't seen this TemplateIP before
        if template_ip not in TemplateComponent._filelist_template_ips:
            TemplateComponent._filelist_template_ips.add(template_ip)
            # Add the TemplateIP's generated filelist using -f directive
            # The path is relative to the wrapper's output directory
            name_list.append(f"-f {prefix}/{template_ip.name}/{template_ip.name}_filelist.f")
    ...
    def verilog_inst(self):
        """
        Override verilog_inst to skip RTL parameters for TemplateComponent.
        
        TemplateComponent modules are pre-configured by ip_builder with their
        parameters already set during the build process. We don't want to
        override these parameters during instantiation.
        
        Returns:
            List of strings representing the Verilog instantiation without parameters
        """
        from operator import concat
        from functools import reduce
        
        # Skip parameter instantiation - directly instantiate with module name and instance name
        str_list = ['%s %s (' % (self.module_name, self.name)]
        
        # Add port connections
        def _Component__eol_append(list_in, common_str, end_str=None):
            """Helper method to append EOL markers (copied from Component)"""
            if end_str is None:
                end_str = common_str
            if list_in:
                result = []
        ...
        str_list += _Component__eol_append(
