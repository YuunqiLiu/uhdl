...
class DArb2ch(Component):
    ...
    def __init__(self, node, pld_type=LwnocBundle):
        ...
        self.out0_req       = pld_type()
        self.out0_ack       = pld_type().reverse()
        ...
        # Forward Arbitration
        self.u_arb = DArb(node, pld_type=LwnocBundle, forward=True)
        SmartAssign(self.u_arb.in_list,     self.in_req_list   )
        SmartAssign(self.u_arb.out0,        self.out0_req      )

        # Backward Decode
        self.u_dec = DDec(node, pld_type=LwnocBundle, forward=False)
